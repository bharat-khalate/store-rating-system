  ...
  const processedRatings = useMemo(() => {
    ...
    if (ratingSortColumn) {
      data = [...data].sort((a, b) => {
        ...
        if (typeof aValue === 'string' && typeof bValue === 'string') {
          return ratingSortDirection === 'asc' ? aValue.localeCompare(bValue) : bValue.localeCompare(aValue);
        }
        if (typeof aValue === 'number' && typeof bValue === 'number') {
          return ratingSortDirection === 'asc' ? aValue - bValue : bValue - aValue;
        }
        return 0;
      });
    }
    ...
    return (
      <div className="flex items-center gap-1">
        {Array.from({ length: 5 }).map((_, idx) => (
          <svg key={idx} className={`w-4 h-4 ${idx < full ? 'text-yellow-500' : 'text-gray-300'}`} fill="currentColor" viewBox="0 0 20 20">
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      ...
      <div className="max-w-4xl mx-auto mb-8">
        <div className="bg-white rounded-lg shadow-lg p-6">
          <div className="flex justify-between items-start mb-6">
            <div>
              <h1 className="text-2xl font-bold text-gray-800 mb-2">Store Dashboard</h1>
            ...
            <div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <p className="text-gray-900">{selectedStore?.storeName || 'N/A'}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Address</label>
                  <p className="text-gray-900">{selectedStore?.address || 'Not provided'}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Overall Rating</label>
                  <div className="flex items-center gap-2">
                    {selectedStore ? renderStars(selectedStore.overallRating) : 'N/A'}
                    <span className="text-sm text-gray-600">{selectedStore?.overallRating?.toFixed(1) ?? ''}</span>
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Owner</label>
                  <p className="text-gray-900">{selectedStore ? `${selectedStore.owner.name} (${selectedStore.owner.email})` : 'N/A'}</p>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-3">
                <div className="bg-blue-50 p-4 rounded-lg">
                  <div className="text-2xl font-bold text-blue-600">{processedRatings.length}</div>
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-6">
          <h2 className="text-2xl font-bold text-gray-800">Ratings Given By Users</h2>
          <p className="text-gray-600 mt-1">List of users who rated this store</p>
        </div>
